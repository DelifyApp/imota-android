...
import com.codedapp.user.data.network.model.Wallet;
...
public class WalletHistoryActivity extends BaseActivity implements WalletHistoryIView {
    ...
    private StringBuilder moneyBuilder;
    private WalletHistoryPresenter<WalletHistoryActivity> presenter =
            new WalletHistoryPresenter<>();
    ...
    public void onSuccess(WalletResponse response) {
        ...
        if (!response.getWallets().isEmpty()) {
            ...
            rvWallet.addItemDecoration(new EqualSpacingItemDecoration(16,
    ...
    private class WalletHistoryAdapter extends RecyclerView.Adapter<MyViewHolder> {
        ...
        public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
           ...
           /* holder.tvAmountVal.setText(String.format("%s %s", SharedHelper.getKey
           (WalletHistoryActivity.this, "currency"),
                    getNewNumberFormat(item.getAmount())));
            holder.tvBalanceVal.setText(String.format("%s %s", SharedHelper.getKey
            (WalletHistoryActivity.this, "currency"),
                    getNewNumberFormat(item.getCloseBalance())));*/

            holder.tvAmountVal.setText(new StringBuilder(moneyBuilder).append(getNumberFormat()
                    .format(item.getAmount())));
            holder.tvBalanceVal.setText(new StringBuilder(moneyBuilder).append(getNumberFormat()
                    .format(item.getCloseBalance())));
        }
