package com.codedapp.user.ui.fragment.invoice;
...
import com.codedapp.user.data.network.model.CheckSumData;
import com.codedapp.user.data.network.model.Datum;
import com.codedapp.user.data.network.model.Message;
import com.codedapp.user.data.network.model.Payment;
import com.codedapp.user.ui.activity.braintree.BrainTreePaymentActivity;
...
import static com.codedapp.user.common.Constants.RIDE_REQUEST.CARD_ID;
import static com.codedapp.user.common.Constants.RIDE_REQUEST.CARD_LAST_FOUR;
...
import static com.codedapp.user.data.SharedHelper.getKey;
import static com.codedapp.user.ui.activity.payment.PaymentActivity.PICK_PAYMENT_METHOD;
...
public class InvoiceFragment extends BaseFragment implements InvoiceIView {
    ...
    private boolean doesCardPaymentActive;
    ...
    public View initView(View view) {
        ...
        try {
            mBrainTreeFragment = BraintreeFragment.newInstance(baseActivity(),
                    BuildConfig.PAYPAL_CLIENT_TOKEN);
    ...
    private void initView(@NonNull Datum datum) {
        ...
        if (SharedHelper.getKey(getContext(), "measurementType").equalsIgnoreCase(Constants.MeasurementType.KM)) {
            if (datum.getDistance() > 1 || datum.getDistance() > 1.0)
                distance.setText(String.format("%s %s", datum.getDistance(),
            ...
            if (datum.getDistance() > 1 || datum.getDistance() > 1.0)
                distance.setText(String.format("%s %s", datum.getDistance(),
                        Constants.MeasurementType.MILES));
            else
                distance.setText(String.format("%s %s", datum.getDistance(),
        ...
        if (datum.getPaymentMode().equals("CARD") && RIDE_REQUEST.get("card_last_four") != null) {
            doesCardPaymentActive = true;
            initPaymentView(datum.getPaymentMode(),
                    String.valueOf(RIDE_REQUEST.get("card_last_four")), true);
        } else {
            doesCardPaymentActive = false;
    ...
    @OnClick({R.id.payment_mode, R.id.pay_now, R.id.done, R.id.tvChange, R.id.tvGiveTip,
            R.id.tvTipAmt})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.payment_mode:
                ...
                intent.putExtra("hideCash", doesCardPaymentActive);
                    ...
                    switch (datum.getPaymentMode()) {
                        ...
                        case Constants.PaymentMode.PAYPAL:
                            PayPalRequest request =
                                    new PayPalRequest(String.valueOf(datum.getPayment().getPayable()))
        ...
        switch (requestCode) {
            ...
            case BRAINTREE_PAYMENT_REQUEST_CODE:
                if (resultCode == RESULT_OK) {
                    String paymentNonce =
                            data.getStringExtra(BrainTreePaymentActivity.PAYMENT_NONCE);
                    ...
                    Toasty.error(getActivity(), getString(R.string.something_went_wrong),
                            Toast.LENGTH_SHORT).show();
                break;
        }
    }
    ...
    public void onPayTmCheckSumSucess(PaytmObject payTmResponse) {
        new PaytmPayment(Objects.requireNonNull(getActivity()), payTmResponse, new PaytmCallback() {
            ...
            public void onPaytmFailure(String errorMessage) {
                Toasty.error(Objects.requireNonNull(getContext()), errorMessage,
                        Toast.LENGTH_SHORT).show();
            }
