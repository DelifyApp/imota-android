package com.codedapp.user.ui.fragment;
...
import com.codedapp.user.base.BaseBottomSheetDialogFragment;
...
public class InvoiceDialogFragment extends BaseBottomSheetDialogFragment {
    ...
    public void initView(View view) {
        StringBuilder currencyBuilder = new StringBuilder()
                .append(SharedHelper.getKey(getActivity(), "currency"))
                .append(" ");
        ...
        if (DATUM != null) {
            ...
            if (payment != null) {
                fixed.setText(new StringBuilder(currencyBuilder)
                        .append(getNewNumberFormat(payment.getFixed())));
                tax.setText(new StringBuilder(currencyBuilder)
                        .append(getNewNumberFormat(payment.getTax())));
                double pastTripTotal = payment.getTotal() + payment.getTips();
                total.setText(new StringBuilder(currencyBuilder)
                        .append(getNewNumberFormat(pastTripTotal)));
                Double payableValue =
                        payment.getTotal() - (payment.getWallet() + payment.getDiscount());
                double pastTripPayable = payableValue + payment.getTips();
                payable.setText(new StringBuilder(currencyBuilder)
                        .append(getNewNumberFormat(pastTripPayable)));
                ...
                if (payment.getWallet() == 0 || payment.getWallet() == 0.0)
                    ...
                    walletDeduction.setText(new StringBuilder(currencyBuilder)
                            .append(getNewNumberFormat(payment.getWallet())));
                }
                if (payment.getDiscount() == 0 || payment.getDiscount() == 0.0)
                    discountLayout.setVisibility(View.GONE);
                else {
                    discountLayout.setVisibility(View.VISIBLE);
                    discount.setText(new StringBuilder(currencyBuilder)
                            .append(getNewNumberFormat(payment.getDiscount())));
                }
                if (payment.getWaitingAmount() == 0 || payment.getWaitingAmount() == 0.0)
                    ...
                    tvWaitingAmount.setText(new StringBuilder(currencyBuilder)
                            .append(getNewNumberFormat(payment.getWaitingAmount())));
                }
                ...
                if (payment.getToll_charge() <= 0) {
                    ...
                    tvTollCharges.setText(new StringBuilder(currencyBuilder)
                            .append(getNewNumberFormat(payment.getToll_charge())));
                }

                if (payment.getRoundOf() != 0) {
                    llRoundOffContainer.setVisibility(View.VISIBLE);
                    tvRoundOff.setText(new StringBuilder(currencyBuilder)
                            .append(getNewNumberFormat(payment.getRoundOf())));
                ...
                if (serviceType != null) {
                    ...
                    switch (serviceCalculator) {
                        case Constants.InvoiceFare.MINUTE:
                            ...
                            timeFare.setText(new StringBuilder(currencyBuilder)
                                    .append(getNewNumberFormat(payment.getMinute())));
                            break;
                        case Constants.InvoiceFare.HOUR:
                            distanceConstainer.setVisibility(View.GONE);
                            timeContainer.setVisibility(View.VISIBLE);
                            timeFare.setText(new StringBuilder(currencyBuilder)
                                    .append(getNewNumberFormat(payment.getHour())));
                            break;
                        case Constants.InvoiceFare.DISTANCE:
                            ...
                            if (payment.getDistance() == 0.0 || payment.getDistance() == 0)
                                ...
                                distanceFare.setText(new StringBuilder(currencyBuilder)
                                        .append(getNewNumberFormat(payment.getDistance())));
                            }
                            break;
