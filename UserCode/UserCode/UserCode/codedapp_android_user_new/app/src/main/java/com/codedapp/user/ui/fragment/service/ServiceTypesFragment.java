...
import com.codedapp.user.common.EqualSpacingItemDecoration;
import com.codedapp.user.data.network.APIClient;
import com.codedapp.user.data.network.model.EstimateFare;
...
import com.codedapp.user.ui.adapter.ServiceAdapter;
...
import static com.codedapp.user.common.Constants.RIDE_REQUEST.DISTANCE_VAL;
import static com.codedapp.user.common.Constants.RIDE_REQUEST.PAYMENT_MODE;
import static com.codedapp.user.common.Constants.RIDE_REQUEST.SERVICE_TYPE;
import static com.codedapp.user.data.SharedHelper.getKey;
import static com.codedapp.user.data.SharedHelper.getProviders;
import static com.codedapp.user.data.SharedHelper.putKey;
...
public class ServiceTypesFragment extends BaseFragment implements ServiceTypesIView {
        ...
        call.enqueue(new Callback<EstimateFare>() {
            ...
            public void onResponse(@NonNull Call<EstimateFare> call,
                ...
                if (response.body() != null) {
                    ...
                    if (object.has("error"))
                        Toast.makeText(baseActivity(), object.optString("error"),
    ...
    public void onSuccess(List<Service> services) {
        ...
        if (services != null && !services.isEmpty()) {
                ...
                AsyncTask.execute(() -> {
                    for (Service s : mServices) {
                        ...
                        if (!TextUtils.isEmpty(s.getMarker()))
                            if (TextUtils.isEmpty(getKey(Objects.requireNonNull(getActivity()),
                                    key))) {
                                Bitmap b =
                                        ((BaseActivity) getActivity()).getBitmapFromURL(s.getMarker());
                                if (b != null)
                                    putKey(getActivity(), key,
                                            ((BaseActivity) getActivity()).encodeBase64(b));
                            }
                    }
                });
            } catch (Exception e) {
            ...
            adapter = new ServiceAdapter(getActivity(), mServices, mListener, capacity,
                    mEstimateFare);
            serviceRv.setLayoutManager(new LinearLayoutManager(getActivity(),
                    LinearLayoutManager.HORIZONTAL, false));
            serviceRv.setItemAnimator(new DefaultItemAnimator());
            serviceRv.addItemDecoration(new EqualSpacingItemDecoration(16,
