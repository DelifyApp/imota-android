package com.codedapp.user.ui.fragment.service_flow;
...
import com.codedapp.user.base.BaseFragment;
import com.codedapp.user.chat.ChatActivity;
import com.codedapp.user.common.CancelRequestInterface;
...
import com.codedapp.user.data.network.model.ProviderService;
import com.codedapp.user.data.network.model.ServiceType;
import com.codedapp.user.ui.activity.main.MainActivity;
import com.codedapp.user.ui.fragment.cancel_ride.CancelRideDialogFragment;
...
import static com.codedapp.user.MvpApplication.showOTP;
...
public class ServiceFlowFragment extends BaseFragment
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.sos:
                ...
                break;
            case R.id.cancel:
                CancelRideDialogFragment cancelRideFragment =
                        new CancelRideDialogFragment(callback);
                cancelRideFragment.show(baseActivity().getSupportFragmentManager(),
                        cancelRideFragment.getTag());
                break;
    ...
    private void initView(Datum datum) {
        ...
        if (provider != null) {
            firstName.setText(String.format("%s %s", provider.getFirstName(),
        ...
        chat.setVisibility(PICKED_UP.equalsIgnoreCase(datum.getStatus()) ? View.GONE :
                View.VISIBLE);
        ...
        shareRideText = getString(R.string.app_name) + ": "
                + datum.getUser().getFirstName() + " " + datum.getUser().getLastName() + " is " +
                "riding in "
        ...
        if (STARTED.equalsIgnoreCase(datum.getStatus())) {
            LatLng source = new LatLng(datum.getProvider().getLatitude(),
                    datum.getProvider().getLongitude());
    ...
    private void sos() {
        new AlertDialog.Builder(getContext())
                ...
                .setCancelable(true)
                .setPositiveButton(getContext().getResources().getString(R.string.yes), (dialog,
                                                                                         which) -> callPhoneNumber(SharedHelper.getKey(getContext(), SOS_NUMBER)))
                .setNegativeButton(getContext().getResources().getString(R.string.no), (dialog,
                                                                                        which) -> dialog.cancel())
                .show();
    }
    ...
    public void update(String eta) {
        isEtaUpdated = true;
        providerEta.setText(String.format("ETA : %s", eta));
    }
        ...
        runnable = () -> {
            try {
                ...
                if (DATUM.getStatus().equalsIgnoreCase(STARTED)
                    ...
                    String lat = String.valueOf(RIDE_REQUEST.get(SRC_LAT));
                    String lng = String.valueOf(RIDE_REQUEST.get(SRC_LONG));
                    if (!lat.equals("null") && !lng.equals("null")) {
                        src = new LatLng(Double.parseDouble(lat), Double.parseDouble(lng));
                    }
                            ...
                            public void onDirectionSuccess(Direction direction, String rawBody) {
                                if (direction.isOK()) {
                                    ...
                                    if (!route.getLegList().isEmpty()) {
                                        ...
                                        String arrivalTime =
                                                String.valueOf(leg.getDuration().getText());
                                        ...
                                        if (!isEtaUpdated)
                                            /*
                                             * Update the ETA only if the driver has not yet reached
                                             * the ride starting location else it will be updated
                                             * from the main activity where it fetches the new route
                                             * record from the source to the destination location.
                                             */
                                            providerEta.setText(String.format("ETA : %s",
                                                    arrivalTime));

                                        System.out.println("RRR src ETA = " + String.format("ETA " +
                                                ": %s", arrivalTime));
                                    }
                                }
                            }
