...
import com.codedapp.driver.data.network.model.ServiceType;
...
import com.codedapp.driver.ui.countrypicker.Country;
import com.codedapp.driver.ui.countrypicker.CountryPicker;
...
import static com.codedapp.driver.common.Constants.APP_REQUEST_CODE;
...
public class RegisterActivity extends BaseActivity implements RegisterIView {
    ...
    public void initView() {
        ...
        if (SharedHelper.getKey(this, Constants.SharedPref.DEVICE_TOKEN).isEmpty()) {
            FirebaseInstanceId.getInstance().getInstanceId().addOnCompleteListener(task -> {
                ...
                SharedHelper.putKey(RegisterActivity.this, Constants.SharedPref.DEVICE_TOKEN,
    ...
    private boolean validation() {
        if (txtEmail.getText().toString().isEmpty()) {
            ...
            Toasty.error(this, getString(R.string.invalid_service_type), Toast.LENGTH_SHORT,
                    true).show();
            return false;
            ...
            Toasty.error(this, getString(R.string.invalid_password_length), Toast.LENGTH_SHORT,
                    true).show();
            return false;
        } else if (txtConfirmPassword.getText().toString().trim().isEmpty()) {
            Toasty.error(this, getString(R.string.invalid_confirm_password), Toast.LENGTH_SHORT,
                    true).show();
            return false;
        } else if (!txtPassword.getText().toString().equals(txtConfirmPassword.getText().toString())) {
            Toasty.error(this, getString(R.string.password_should_be_same), Toast.LENGTH_SHORT,
                    true).show();
            return false;
        } else if (!chkTerms.isChecked()) {
            Toasty.error(this, getString(R.string.please_accept_terms_conditions),
    ...
    private void register(String countryCode, String phoneNumber) {
        ...
        map.put("device_token", toRequestBody(SharedHelper.getKeyFCM(this,
                Constants.SharedPref.DEVICE_TOKEN)));
        map.put("device_id", toRequestBody(SharedHelper.getKeyFCM(this,
                Constants.SharedPref.DEVICE_ID)));
        ...
        if (!txtReferalCode.getText().toString().trim().isEmpty())
            map.put("referral_code", toRequestBody(txtReferalCode.getText().toString().trim()));
...
//                onBackPressed();
    ...
    private void setupSpinner(@Nullable SettingsResponse response) {
        ArrayList<String> lstNames = new ArrayList<>(response != null ?
                response.getServiceTypes().size() : 0);
    ...
    public void fbPhoneLogin(String strCountryCode, String strCountryISOCode,
                    ...
                    public void onSuccess(Account account) {
                        ...
                        if (AccountKit.getCurrentAccessToken().getToken() != null) {
                            ...
                            SharedHelper.putKey(RegisterActivity.this,
                                    Constants.SharedPref.DIAL_CODE,
                                    "+" + phoneNumber.getCountryCode());
                            SharedHelper.putKey(RegisterActivity.this, Constants.SharedPref.MOBILE,
                                    phoneNumber.getPhoneNumber());
                            register(SharedHelper.getKey(RegisterActivity.this,
                                    Constants.SharedPref.DIAL_CODE),
