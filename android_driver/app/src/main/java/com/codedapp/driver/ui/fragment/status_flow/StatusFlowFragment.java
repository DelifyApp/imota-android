...
import com.codedapp.driver.base.BaseFragment;
...
import com.codedapp.driver.data.network.model.Request_;
import com.codedapp.driver.data.network.model.TimerResponse;
import com.codedapp.driver.data.network.model.TripResponse;
import com.codedapp.driver.ui.activity.main.MainActivity;
import com.codedapp.driver.ui.bottomsheetdialog.cancel.CancelDialogFragment;
...
public class StatusFlowFragment extends BaseFragment implements StatusFlowIView {
    ...
    private void init() {
        ...
        if (data != null && data.getStatus() != null) {
            ...
            if (tripResponse != null && tripResponse.getProviderDetails() != null)
                ...
                currentLocation = new LatLng(Double.parseDouble(SharedHelper.getKey(getContext(),
                        "latitude")),
            ...
            if (data.getStatus().equalsIgnoreCase(Constants.checkStatus.ACCEPTED) ||
                ...
                ((MainActivity) getContext()).drawRoute(currentLocation, origin, false);
            else ((MainActivity) getContext()).drawRoute(origin, destination, false);
        }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.imgCall:
                ...
                break;
            case R.id.btnCancel:
                SharedHelper.putKey(thisContext, Constants.SharedPref.CANCEL_ID,
    ...
    void cancelRequestPopup() {
        try {
            ...
            alertDialogBuilder
                    ...
                    .setCancelable(false)
                    .setPositiveButton(thisContext.getResources().getString(R.string.yes),
                            (dialog, id) -> {
                                try {
                                    CancelDialogFragment cancelDialogFragment =
                                            new CancelDialogFragment();
                                    cancelDialogFragment.show(getActivity().getSupportFragmentManager(),
                                            cancelDialogFragment.getTag());
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                            }).setNegativeButton(thisContext.getResources().getString(R.string.no),
    ...
    private void showOTP() {
        ...
        submitBtn.setOnClickListener(view1 -> {
            ...
            if (data.getOtp().equalsIgnoreCase(pinView.getText().toString())) {
                try {
                    if (thisContext != null)
                        Toasty.success(thisContext,
                                thisContext.getResources().getString(R.string.otp_verified),
                ...
                if (thisContext != null && isAdded())
                    Toasty.error(thisContext,
                            thisContext.getResources().getString(R.string.otp_wrong),
                            Toast.LENGTH_SHORT).show();
                else
                    Toasty.error(thisContext,
                            thisContext.getResources().getString(R.string.otp_wrong),
    ...
    private void showAddTollDialog() {
        ...
        submitBtn.setOnClickListener(view1 -> {
            if (!toll_amount.getText().toString().trim().isEmpty()
                    && Double.parseDouble(toll_amount.getText().toString().trim()) > 0) {
                addTollDialog.dismiss();
                tollAmount = Double.valueOf(toll_amount.getText().toString().trim());
                customHandler.removeCallbacks(updateTimerThread);
                statusUpdateCall(STATUS);
            } else {
                if (Objects.requireNonNull(getActivity()) != null) {
                    Toast.makeText(Objects.requireNonNull(getActivity()),getResources().getString(R.string.valid_amount), Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void dismissDialog() {
        if (addTollDialog != null) {
            if (addTollDialog.isShowing()) {
                addTollDialog.dismiss();
            }
        }
    }
